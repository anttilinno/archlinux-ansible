...
- **print**: CUPS for the HP Neverstop Laser over wifi. Off by default (`print_enabled`). Default path is driverless — `cups` plus `avahi-daemon.service`; CUPS 2.x auto-creates a temporary queue for the DNS-SD-discovered printer. `nss-mdns` and `ghostscript` are in `print_packages` because both fail silently without it — see the notes below. `print_scanner_enabled` (off) adds sane-airscan + simple-scan for eSCL scanning on the MFP 1200 models; the 1000/1000w are print-only. `print_hplip_enabled` (off) is the fallback; it drags in `hplip-plugin` from the AUR because the Neverstop PPDs are marked `requires proprietary plugin` — hplip without the plugin gives a queue that accepts jobs while the printer stays silent.
